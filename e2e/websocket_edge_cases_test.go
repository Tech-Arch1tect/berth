...
func TestWebSocketInsufficientAccessDeniedAfterAuth(t *testing.T) {
	...
	assert.Equal(t, http.StatusForbidden, resp.StatusCode,
		"a valid key authenticates, but the handler-level access check must still deny a server the owner cannot reach; body=%s", resp.GetString())
}
